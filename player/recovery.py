        ...
        class Handler(http.server.BaseHTTPRequestHandler):
            ...
            def do_GET(self):
                ...
                if self.path == '/setup/state':
                    return self.reply(200, {'csrf': portal.token})
            ...
            def do_POST(self):
                if not self.trusted() or self.path != '/setup' or self.headers.get('Origin') != 'http://' + IP or self.headers.get('X-Setup-Token') != portal.token:
                    return self.reply(403, {})
                try:
                    size = int(self.headers.get('Content-Length', '0'))
                    if not 0 < size <= 2048:
                        return self.reply(413, {})
                    value = validate_wifi(json.loads(self.rfile.read(size)))
                    if portal.submitted:
                        return self.reply(409, {})
                    portal.pending.put_nowait(value)
                    portal.submitted = True
                    self.reply(202, {'ok': True})
                except (ValueError, TypeError, OSError, queue.Full):
                    self.reply(400, {'error': 'Check the Wi-Fi details'})
        return Handler
...
def window(value):
    ...
    try:
        ...
        while time.monotonic() < end and credentials() == value and dns.poll() is None:
            try:
