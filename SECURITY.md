...
- [Hidden recovery Wi-Fi](docs/player-recovery.md) is a time-boxed root service, not an invisible or inherently secure network. Its WPA2 password is generated by the server, encrypted in the database, delivered only to that approved player, and revealed only on explicit administrator request. Anyone who knows that password and is within range can change Wi-Fi during a recovery window. Back up `provisioning.key`, do not commit `recovery.json`, and validate radio/firewall behavior on real hardware. An offline stolen player cannot receive revocation.
